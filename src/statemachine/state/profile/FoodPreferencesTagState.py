...
class FoodPreferencesTagState(State):
    def __init__(self, context):
        ...
        self.options = tags.preferencesTags
        self.hasPoll = True

    def processUpdate(self, update: Update):
        message = update.getMessage()

        if self.context.user.preferences_tags is not None and message is not None and message.text == self.context.getMessage("preferences_skipBtn"):
        ...
        if poll_answer and int(poll_answer.poll_id) == int(self.context.user.active_poll_id):
            ...
            for option_id in poll_answer.option_ids:
                self.context.user.preferences_tags.add(self.options[option_id])
            self.context.setState(profile.RestrictionsTagState(self.context))
            self.context.saveToDb()
        self.hasPoll = False

    async def sendMessage(self, update: Update):
        options = list(map(lambda x: self.context.getMessage(x), tags.preferencesTags))

        kb = [
            [types.KeyboardButton(text=self.context.getMessage("preferences_skipBtn"))],
        ]
