...
class DietsTagState(State):
    ...
    async def process_update(self, update: Update):
        ...
        if (
            ...
            and message.text == self.context.get_message("diets_skipBtn")
        ):
            self.context.set_state(profile.InterestsTagState(self.context))
        ...
        kb = [
            [
                types.KeyboardButton(
                    text=self.context.get_message("diets_skipBtn")
                )
            ],
        ]
