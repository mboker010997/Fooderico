...
class DBController:
    ...
    def deleteUser(self, chat_id):
        ...
        self.cursor.execute(
            f"DELETE FROM tele_meet_relations WHERE user_id={id} or other_user_id={id}"
        )
        # print(self.cursor.fetchall())
    ...
    def createQuery(self, table_name, columns: dict):
        ...
        args = ",".join(args)
        self.cursor.execute(
            f"CREATE TABLE IF NOT EXISTS {table_name} ({args})"
        )
        self.connection.commit()
    ...
    def updateUserInformation(self, id, update_fields):
        if id is None:
            ...
            values = ", ".join(
                [
                    f"{update_fields[key]}"
                    if isinstance(update_fields[key], int)
            ...
            self.cursor.execute(
                f"UPDATE {self.table_name} SET {set_field} WHERE id = {id};"
            )
        self.connection.commit()

    def updateState(self, chat_id, state):
        return self.updateUserInformation(
            self.getIdByChatId(chat_id),
    ...
    def getUser(self, id):
        ...
        if user_info:
            ...
            if user.others_interests is not None:
                user.others_interests = " ".join(
                    user.others_interests.split(" ")
                )
    ...
    def setUser(self, user):
        ...
        if update_fields.get("preferences_tags", None) is not None:
            update_fields["preferences_tags"] = ",".join(
                list(user.preferences_tags)
            )
        if update_fields.get("restrictions_tags", None) is not None:
            update_fields["restrictions_tags"] = ",".join(
                list(user.restrictions_tags)
            )
        ...
        if update_fields.get("interests_tags", None) is not None:
            update_fields["interests_tags"] = ",".join(
                list(user.interests_tags)
            )
    ...
    def getIdByChatId(self, chat_id):
        ...
        self.cursor.execute(
            f"SELECT id FROM {self.table_name} WHERE chat_id = '{chat_id}';"
        )
    ...
    def getChatIdById(self, id):
        ...
        self.cursor.execute(
            f"SELECT chat_id FROM {self.table_name} WHERE id = '{id}';"
        )
        result = self.cursor.fetchone()
        return result[0] if result else None
    ...
    def getUserTags(self, chat_id):
        ...
        self.cursor.execute(
            f"SELECT id, {self.tags_for_matching} FROM {self.table_name} WHERE id={id}"
        )
        return self.cursor.fetchone()

    def getTags(self):
        self.cursor.execute(
            f"SELECT id, {self.tags_for_matching} FROM {self.table_name}"
        )
        return self.cursor.fetchall()

    def getUserRelationsIds(self, id):
        self.cursor.execute(
            f"SELECT other_user_id FROM tele_meet_relations WHERE user_id={id}"
        )
        return self.cursor.fetchall()

    def getUserStatus(self, id):
        self.cursor.execute(
            f"SELECT status FROM {self.table_name} WHERE id={id}"
        )
        return self.cursor.fetchone()

    def getUserMatchesIds(self, id):
        self.cursor.execute(
            f"SELECT other_user_id FROM tele_meet_match WHERE user_id={id}"
        )
        return self.cursor.fetchall()
